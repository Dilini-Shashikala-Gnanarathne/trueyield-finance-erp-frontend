      ...
      {user && (
        <div className="sidebar__user-panel">
          <NavLink to="/profile" className={({ isActive }) => "sidebar__user" + (isActive ? " active" : "")}>
            <div className="sidebar__user-avatar">{ROLE_ICON[user.role] ?? "👤"}</div>
            <div className="sidebar__user-info">
              <span className="sidebar__user-name">{user.fullName}</span>
              <span className="sidebar__user-role">{user.role}</span>
            </div>
          </NavLink>
          <button id="sidebar-logout" type="button" className="sidebar__logout-btn" title="Sign Out" onClick={handleLogout}>
            🚪
          </button>
        </div>
      )}
